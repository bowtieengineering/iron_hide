...
module IronHide
  ...
  class ObligationScope < ::ActiveRecord::Relation
    ...
    def model_for (path)
      ...
      if OblicationScope.is_a_association_proxy?(reflection)
    ...
    def map_reflection_for( path )
      ...
      reflection = path.empty? ? top_level_model : begin
        ...
        if !ObligationScope.is_a_association_proxy?(parent) and parent.respond_to?(:klass)
      ...
      if !ObligationScope.is_a_association_proxy?(reflection) and !reflection.respond_to?(:proxy_scope) and reflection.is_a?(::ActiveRecord::Reflection::ThroughReflection)
